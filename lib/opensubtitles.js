...
async function searchSubtitles(filename) {
    const dbApiKey = (0, db_1.getConfig)("opensubtitles_api_key");
    ...
    let query = path_1.default.basename(filename, path_1.default.extname(filename));
...
async function downloadSubtitle(file_id, savePath) {
    const dbApiKey = (0, db_1.getConfig)("opensubtitles_api_key");
    const API_KEY = dbApiKey || process.env.OPENSUBTITLES_API_KEY;
    if (!API_KEY) {
        throw new Error("OPENSUBTITLES_API_KEY is not configured.");
    ...
    // 3. Save to disk
    fs_1.default.writeFileSync(savePath, content, "utf-8");
}
