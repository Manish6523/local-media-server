...
function getDb() {
  ...
  `);
    ...
    const matcherColumns = new Set(sqliteDb.prepare("PRAGMA table_info(media_files)").all().map(c => c.name));
    for (const [column, definition] of Object.entries({ imdb_id: "TEXT", type: "TEXT", season: "INTEGER", episode: "INTEGER", episode_end: "INTEGER", match_source: "TEXT" })) {
        if (!matcherColumns.has(column))
            sqliteDb.exec(`ALTER TABLE media_files ADD COLUMN ${column} ${definition}`);
    }
...
function upsertMedia(entry) {
    ...
    if (entry.type === "movie") {
        db.delete(schema.episodes).where((0, drizzle_orm_1.eq)(schema.episodes.mediaAssetId, assetId)).run();
        ...
        db.delete(schema.movies).where((0, drizzle_orm_1.eq)(schema.movies.mediaAssetId, assetId)).run();
        // Upsert tvShow
        let tvShow = db.select().from(schema.tvShows).where(entry.omdb_id ? (0, drizzle_orm_1.eq)(schema.tvShows.omdbId, entry.omdb_id) : (0, drizzle_orm_1.eq)(schema.tvShows.title, entry.title)).get();
        if (!tvShow) {
